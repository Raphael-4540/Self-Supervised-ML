...
%
% The demo images and mask files are incorporated as part of the demo
% package on GitHub. https://github.com/Raphael-4540/Self-Supervised-ML
%
...
% time-lapse imagery used for the SSL clumped cell mask.
% 2) The nuclei mask be stored as a binary tiff (included with demo package)
% 3) The output .mat file of the SSL code (included with demo package)
% 4) Original tiff microscopy imagery for overlays (included with demo
%   package)
%
% Functions called:
...
%
% Output:
%  Declumped cell mask saved as a *.mat file
% 
...
% USER INPUT REQUIRED!!
prompt = 'Which part of Figure 5 do you want to reproduce? [ab/c/d] \n';
str = input(prompt,'s');
if strcmp(str,'ab')
    fig_dir = '\Fig5\ab';
elseif strcmp(str,'c')
...
% If you want to process your own data change dir_str below:
% 1.  Follow the rules from the SSL_Demo for files and directories!
% 2.  The cell segmentation mask will need to be created via SSL_Demo.
% 3.  You will also need to put a nuclei mask in the ../mask sub-directory.
dir_str = [ install_dir fig_dir '\data'];
...
%
filename = [ install_dir fig_dir '\mask\data_cell_mask.mat'];
load(filename); % SSL cell mask in your current MATLAB directory
Mask = imread([install_dir fig_dir '\mask\nuclei mask.tif']); %binary mask of nucleus in your current MATLAB directory
...
for k=1:n_Lnc
    ...
    if NumOfRegs > 1
        ...
        disp(['Checking assignments for cell index = ' num2str(k)]);
...
% Store de-clumped cell boundaries.
filename = [ install_dir fig_dir '\mask\data_cell_mask_declumped.mat'];
save(filename,'BL')
disp(['Declumpled Cell Mask saved to file: ' filename])
