...
%
% One step instructions for working with incoporporated test imagery: 
% Press 'Run' and respond to prompts in the Command Window.
%
% For your own data, there is an additional step in which you first need to
% add your images to the "../UserData/data" folder provided before executing the
% Run command.
% 
% For first time users, we also recommend evaluating the code section by section
% (Ctrl+Enter) and reading the section comments which incorporate
...
%
% Functions called:
%   UserInput_SSL_Demo(): Interactive prompts for locating data
...
install_dir = pwd;
%  User input.
dir_str = UserInput_SSL_Demo(install_dir);

% FOR PROCESSING YOUR OWN DATA:
% Add your images to the "../UserData/data" folder provided
...
for k = 2:NumOfFrames % Select a frame.
    ...
    disp(['Training on frame ' num2str(k)])
end
toc
...
save([dir_str(1:idx(size(idx,2))) 'mask\' file_out_name_mat], 'BW_cube_fv');

disp('***');
disp(['Segmentation Mask saved to folder:   ' dir_str(1:idx(size(idx,2))) 'mask']);
disp(['Segmentation Video saved to folder:  ' v.Path]);
disp('***');
